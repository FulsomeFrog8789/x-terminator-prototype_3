...
var monkey , monkey_idle, monkey_run
...
function preload(){
  ...
  monkey_idle =            loadAnimation("froge.png")
  monkey_run = loadAnimation("froge_go_zoom.png")
...
function setup() {
  ...
  //creating monkey
   ...
   monkey.addAnimation("moving", monkey_idle);
   monkey.debug = true
...
//}
  ...
  if(enemy.isTouching(attack) && keyDown("z")){
    enemy.lifetime = 0;
  }

    if(keyDown("space") && (monkey.y>=527. || monkey.y==417.3)){
      monkey.velocityY = -18;
    }

    

    if(keyDown("space") && keyDown("down_arrow") && (monkey.y>=527. || monkey.y==417.3)){
      monkey.velocityY = -25;
    }
